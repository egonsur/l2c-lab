...
const WIFI={
 ...
 costs:{p2p:1500,fiber:6000,budget:8000},
...
function wifiEvaluate(state){
 ...
 const cost=state.aps.reduce((n,a)=>n+WIFI.generations[a.generation].cost+WIFI.apTypes[a.type].cost,0)+(state.p2p?WIFI.costs.p2p:0)+(state.fiber?WIFI.costs.fiber:0);
 ...
 return {rooms:active,aps:loads,connected:active.filter(r=>r.hasInternetConnectivity).length,users,coveredUsers,totalScenarioUsers:roomsTotal,buildingBWifi,buildingBOnline,buildingBPath:hasBuildingUpstream,cost,contention,overloaded:loads.filter(a=>a.overloaded).length,bandCompat,
...
function wifiWhy(state){const e=wifiEvaluate(state),lessons=[];if(e.indoor>=.9&&e.capacity<.7)lessons.push(['Coverage is not capacity','Most rooms have a signal, but one or more APs serve too many users. Coverage and capacity are different design questions.']);if(state.aps.length>3)lessons.push(['More is not always better','Additional APs can improve capacity, but nearby APs sharing a channel can create contention. Placement and channel planning matter.']);if(state.aps.some(a=>a.band==='6'))lessons.push(['Client devices matter','6 GHz adds spectrum, but only a portion of this fictional school’s devices can use it. Keeping 5 GHz available preserves compatibility.']);if(state.p2p)lessons.push(['Use the right tool','The directional P2P bridge connects networks between buildings. Indoor APs and a secondary switch serve Building B users.']);if(state.fiber)lessons.push(['Wi-Fi does not create Internet capacity','A fiber building link improves the uplink, but the school’s 500 Mbps Internet service still limits the whole campus.']);if(e.contention)lessons.push(['Channels are shared air','Nearby APs on one channel must share airtime. Reuse channels deliberately and avoid unnecessary overlap.']);if(!e.buildingBOnline)lessons.push(['Building B needs an upstream and local network','A bridge or fiber link alone does not provide client Wi-Fi. Building B also needs a switch and local AP.']);return lessons.length?lessons:[['A good campus architecture','Use indoor APs for users, outdoor APs for outdoor areas, and a directional bridge or fiber link between buildings.']];}
